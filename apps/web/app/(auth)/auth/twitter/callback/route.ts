...
export async function GET(request: Request) {
  ...
  if (!code || !state) {
    return redirectTo("/app/integrations?twitter=error&reason=missing_params");
  }
  ...
  const res = await fetch(`${API_URL}/api/integrations/twitter/callback`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    ...
    return redirectTo(
      `/app/integrations?twitter=error&reason=${encodeURIComponent(reason)}`,
    );
  }

  return redirectTo("/app/integrations?twitter=connected");
}
