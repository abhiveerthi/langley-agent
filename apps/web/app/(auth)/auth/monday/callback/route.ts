...
export async function GET(request: Request) {
  ...
  if (oauthError) {
    return redirectTo(`/app/integrations?monday=error&reason=${encodeURIComponent(oauthError)}`);
  }
  if (!code || !state) {
    return redirectTo("/app/integrations?monday=error&reason=missing_params");
  }
  ...
  const res = await fetch(`${API_URL}/api/integrations/monday/callback`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    ...
    return redirectTo(`/app/integrations?monday=error&reason=${encodeURIComponent(reason)}`);
  }

  return redirectTo("/app/integrations?monday=connected");
}
